...
<body class="sidebar-mini layout-fixed" style="height: auto;" onmousemove="bodycheck()">
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
            ...
            <ul class="navbar-nav">
                ...
                $getCounselorActive = Auth::user();
                 ?>
                @if($sessionCounselorid != 1)
                  @if($getCounselorActive->counselor_availability == 0)
                  <p class="activeButtin" id="activeButton"><input type="button" class="btn btn-danger" name="availability" onclick="myFunctionActivation(this);" value="Offline" id="active"> &nbsp;&nbsp; <label><?php echo $getCounselorActive->name;?></label></p>

                  <p class="activeButtin" id="inactivebutton" style="display:none;"><input type="button" class=" btn btn-primary" name="availability" onclick="myFunctionActivation(this);" value="Online" id="inactive"> &nbsp;&nbsp;<label><?php echo $getCounselorActive->name;?></label></p>

                  @else
                    <p class="activeButtin" id="inactivebutton"><input type="button" class=" btn btn-primary" name="availability" onclick="myFunctionActivation(this);" value="Online" id="inactive"> &nbsp;&nbsp;<label><?php echo $getCounselorActive->name;?></label></p>

                    <p class="activeButtin" id="activeButton" style="display:none;"><input type="button" class="btn btn-danger" name="availability" onclick="myFunctionActivation(this);" value="Offline" id="active"> &nbsp;&nbsp; <label><?php echo $getCounselorActive->name;?></label></p>
                  @endif 
                @endif
                <?php   
                $sessionCounselorid = Auth::user()->id;
        ...
        <div class="content-wrapper" style="min-height: 917px;">
            ...
            <section class="content" style="padding-top: 20px">
                    ...
                    <div class="row mb-2" id="alertMSGInactive" style="display:none;">
                        <div class="col-lg-12">
                           <!--  <div class="alert alert-success" role="alert">{{ session('message') }}</div> -->
                             <div class="alert alert-success" role="alert">Available for sessions</div>
                        </div>
                    </div>

                    <div class="row mb-2" id="alertMSGActive" style="display:none;">
                        <div class="col-lg-12">
                           <!--  <div class="alert alert-success" role="alert">{{ session('message') }}</div> -->
                             <div class="alert alert-success" role="alert">Not available for sessions</div>
                        </div>
                    </div>
...
<script>
     ...
     var firebaseConfig_1 = {
        ...
        databaseURL: 'http://65.2.178.96/phpmyadmin/index.php?route=/database/structure&db=tam_web_staging',
    ...
    const messaging_1 = firebase.messaging();
    function startFCM() {
        messaging_1
            .requestPermission()
            .then(function () {
  ...
  $.fn.dataTable.ext.classes.sPageButton = '';
    ...
    <script>
function myFunctionActivation(e)
  {
    alertRemove();
    var thisUserId = $(e).val();
    
    if(thisUserId == "Offline")
    {
      var  thisStatus = 1;
    }else{
      var  thisStatus = 2;
    ...
    $.ajax({
            ...
            success: function(data) {
              // location.reload(); activeButton inactivebutton
              // alertMSGActive alertMSGInactive
               if(thisUserId == "Offline")
                {
                  $('#activeButton').css('display','none');
                  $('#inactivebutton').css('display','block');
                  $('#alertMSGInactive').css('display','block');
                   setTimeout(alertRemove, 10000);
                }

                if(thisUserId == "Online"){
                  $('#activeButton').css('display','block');
                  $('#inactivebutton').css('display','none');
                  $('#alertMSGActive').css('display','block');
                  setTimeout(alertRemove, 10000);
                }
            }
        });
  ...
  function alertRemove(){
    $('#alertMSGInactive').css('display','none');
    $('#alertMSGActive').css('display','none');
  }
    ...
    <script>
            ...
            const FULL_DASH_ARRAYBody = 283;

            let TIME_LIMITBody = 1800;
            let timePassedBody = 0;
            let timeLeftBody = TIME_LIMITBody;
            let timerIntervalBody = null;

            var checkCountbody  = 1;
            startTimerBody();

        function bodycheck(){
            checkCountbody+1;
            console.log(checkCountbody);
            timePassedBody = 0;
            onTimesUpBody();
        }

        function onTimesUpBody() {
          clearInterval(timerIntervalBody);
          startTimerBody();
        }

        function startTimerBody() {
          timerIntervalBody = setInterval(() => {
            timePassedBody = timePassedBody += 1;
            timeLeftBody = TIME_LIMITBody - timePassedBody;
        ...
        function calculateTimeFractionBody() {
          const rawTimeFraction = timeLeftBody / TIME_LIMITBody;
          return rawTimeFraction - (1 / TIME_LIMITBody) * (1 - rawTimeFraction);
        }
