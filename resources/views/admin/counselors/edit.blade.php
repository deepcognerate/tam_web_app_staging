...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.counselors.update", [$counselor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="feature_name">Feature</label>
                       
                <select class="form-control select2 {{ $errors->has('feature_name') ? 'is-invalid' : '' }}" name="feature_id[]" id="feature_id" multiple required>
                @if(!empty($categorys))   
                @foreach($feature as $id => $features)
                        <option value="{{ $features->id }}" 
                            <?php if(in_array($features->id,$multiFeature)){ echo 'selected'; }?>>{{ $features->feature_name }} </option>
                    @endforeach
                @endif
                </select>
