...
<link rel="stylesheet" href="{{asset('public/chatboat/assets/vendors/mdi/css/materialdesignicons.min.css')}}">
  ...
  <div class="card">
      <div class="card-header">
          Chat for User: @if(!empty($userData->name)) {{ $userData->name }} @else  @endif
      </div>
      <div class="card-body">
      
        @if(!empty($liveChats))
                @foreach($liveChats as $chatHistory)

                <?php  
                    $date = date('Y-m-d',strtotime($chatHistory->created_at));
...
setTimeout(loadAdminViewPage, 10000);
  function loadAdminViewPage(){    
    location.reload();
  }
</script>
