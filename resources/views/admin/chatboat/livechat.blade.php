...
<div class="chat-box-body">
  <div class="chat-logs chat-history">
    ...
    @if(!empty($chat_historylastThree))
      @foreach($chat_historylastThree as $key => $chat_historylast)
      
    @if(!empty($chat_historylast->chat_history))
      <?php $olddates = array(); ?>
      @foreach($chat_historylast->chat_history as $key12 => $chat_s)
    
      <?php 
        
        $date = date('Y-m-d',strtotime($chat_s->created_at));
        $time = date('H:i:s',strtotime($chat_s->created_at));

      if(!in_array( $date ,$olddates)){

        
        $olddates[] = $date;

        echo '<div id="cm-msg-2" class="chat-msg user" >  </div><div id="cm-msg-1" class="chat-msg self" ></div>
        <label style="text-align: center !important;padding: 1px;background-color: #8383838f;width: 100%;">'.$date.'</label> <div id="cm-msg-2" class="chat-msg user" >  </div><div id="cm-msg-1" class="chat-msg self" ></div></br>';
      }
      ?>
      @if($chat_s->status == 1)
      <div id="cm-msg-2" class="chat-msg user" >          
          ...
          <div class="cm-msg-text"><span>{{ $chat_s->message }}</span></br><small>{{ $date }} {{ $time }} <i class="fa fa-check iconTickRight" aria-hidden="true"></i></small></div>        
      </div>
      @else
            @if($chat_s->msgType == 1)
            <div id="cm-msg-1" class="chat-msg self" >
                ...
                <div class="cm-msg-text"><span><embed src="{{asset('public/chatAttachment/')}}/<?php echo $chat_s->chatAttachment; ?>" width= "300" height= "300"></span></br><small style="float:right;">{{ $date }} {{ $time }} <i class="fa fa-check iconTickRight" aria-hidden="true"></i></small></div>        
            </div>
            @endif
            @if($chat_s->message != 'attachment')
            <div id="cm-msg-1" class="chat-msg self" >
                <span class="msg-avatar">
                    <img src="https://i.stack.imgur.com/l60Hf.png">
                </span>          
                <div class="cm-msg-text"><span>{{ $chat_s->message }}</span></br><small style="float:right;">{{ $date }} {{ $time }} <i class="fa fa-check iconTickRight" aria-hidden="true"></i></small></div>        
            </div>
            @endif
        @endif
      @endforeach 
    ...
    <?php 
      echo '<div id="cm-msg-2" class="chat-msg user" >  </div><div id="cm-msg-1" class="chat-msg self" ></div>
        <label style="text-align: center !important;padding: 1px;background-color: #83838300;width: 100%;">Counsellor Category - '.$chat_historylast->getCategory->category_name.'</br> Counsellor Remarks - '.$chat_historylast->close_remark.' , '.$chat_historylast->close_issue_code.','.$chat_historylast->close_reason.'</label> <div id="cm-msg-2" class="chat-msg user" >  </div><div id="cm-msg-1" class="chat-msg self" ></div> </br>';
    ...
    <?php 
        ...
        $date = date('Y-m-d',strtotime($liveChat->created_at));
        $time = date('H:i:s',strtotime($liveChat->created_at));
      ...
      if(!in_array( $date ,$dates)){
        ...
        $dates[] = $date;
        echo '<div id="cm-msg-2" class="chat-msg user" >  </div><div id="cm-msg-1" class="chat-msg self" ></div>
        <label style="text-align: center !important; padding: 1px; background-color: #8383838f;
    width: 100%;">'.$date.'</label> <div id="cm-msg-2" class="chat-msg user" >  </div><div id="cm-msg-1" class="chat-msg self" ></div></br>';
      }
    ...
    <div id="cm-msg-2" class="chat-msg user" >          
        ...
        <div class="cm-msg-text"><span>{{ $liveChat->message }}</span></br><small>{{ $date }} {{ $time }} </small></div>        
    </div>
    @else
    ...
    <div id="cm-msg-1" class="chat-msg self" >
        ...
        <div class="cm-msg-text"><span><embed src="{{asset('public/chatAttachment/')}}/<?php echo $liveChat->chatAttachment; ?>" width= "300" height= "300"></span></br><small style="float:right;">{{ $date }} {{ $time }} <i class="fa fa-check iconTickRight" aria-hidden="true"></i> </small></div>        
    </div>
    @endif
        ...
        <div id="cm-msg-1" class="chat-msg self" >
            ...
            <div class="cm-msg-text"><span>{{ $liveChat->message }}</span></br><small style="float:right;">{{ $date }} {{ $time }} <i class="fa fa-check iconTickRight" aria-hidden="true"></i></small></div>        
        </div>
        @endif
    @endif
    @endforeach 
...
@csrf
    ...
    <input class="form-control" type="hidden" name="counsellor_id"  id="counselor_id" value=" @if(!empty($getLiveChats->counsellor_id)) {{ $getLiveChats->counsellor_id }} @else @endif">
    ...
    <!-- <input type="file" name="file" id="attachmentids" style="display: none;"> -->
    ...
    <input type="hidden" id="filecountIc" value="0">
...
<button class="btn btn-chat-footer btn-sm" id="myBtn">Escalate</button>
    ...
    <div id="myModal1" class="modal1">
      ...
      <div class="modal-content1">
        <form  method="post" id="chatCloseAjax_ids" action="@if(!empty($getLiveChats->getUser->id)) {{ route('admin.chat-closed-live.closeChatLive', $getLiveChats->getUser->id) }} @else  @endif " enctype="multipart/form-data">
        @csrf
...
<script>
    ...
    $(document).on('submit', 'form#chatCloseAjax_ids', function (e) {
